...
- 自动生成各个说明文档（可借助rest api），这些生成脚本不应该删除已有的介绍内容，例如对狗狗行为规则的介绍
- 拿着骨头右键狗是喂食，否则是攻击
- 攻击是近程攻击，想想怎么实现：显示一个红色1/4扇环，固定位置，与发动者以及它宠物/主人不相撞，与其他精灵相撞并产生伤害
  - 看来需要对所有主人的ID进行哈希，产生group的ID，或者更精细的控制，看看能不能记录精灵以其其主人的树形结构，随后控制攻击环不与指定精灵碰撞（树上的其他节点）
  - 添加视觉范围、攻击范围（扇环外扇形的半径）、精灵宽度占图像宽度的比例、精灵高度占图像高度的比例（用于寻路算法）等属性
- 或者更简单的攻击方法，点击对方，到达附近后产生交互事件，后端首先判断位置近不近（因为对方也会移动，也同时是为了防止作弊），随后决定是攻击，还是什么别的事件例如喂食？至少血量减少的通知，就直接用通知的形式告诉攻击者吧，哈哈，懒得弄复杂的动画。这样的话，甚至物理引擎都可以去掉了，因为不需要检测碰撞
