...
- 虽然对数据库的并发修改的原子性和隔离性由@Transactional注解保证，但是对于缓存的并发修改，目前还没有保证。之前尝试加锁，但导致死锁、性能下降和编程复杂，为此，计划使用rabbitmq消息队列来实现串行化，客户端向服务端的消息发送消息，当然，服务端的任意模块也可以作为生产者向消息队列发送消息。服务端的各个模块作为消费者消费消息。这些模块高内聚低耦合，使得不需要加锁
- 前端是否也应该加消息队列实现串行化？
