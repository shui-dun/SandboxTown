...
- 目前各种特殊效果例如火焰附体、烧伤是硬编码在攻击等逻辑之中，之后改成装饰器模式（例如用火焰附体装饰器来装饰精灵）可扩展性更好。
- 简化设计，废弃整个物理引擎（因为bug太多了，不仅要解决自己代码的bug，还要解决物理引擎的上游bug），进而，可以简化前后端交互（废弃COORDINATE request，服务器不再需要通过前端获得位置信息）
- 设计生态环境：目前建筑随机分散在地图各处，没有什么规律，未来希望改为以生态环境为单位，一个生态环境包含各个建筑和生物，一个生态环境中的各个建筑之间有机的结合在一起，有一定规律性又有一定随机性，例如在我的世界中，村庄是一个生态环境，村庄具有一定随机性，例如各个村庄的形状和房屋分布有所不同，但是村庄又有一定规律性，例如房屋之间总之用街道相互连接在一起，村庄的中心总是有钟，考虑使用以下方式实现：
  - 使用Perlin噪声生成地形，例如湖泊、草地、沙漠，每个地图点根据值的大小决定地形类型。另外，使用用瓦片地图，实现地形边界的平滑过渡（去网上找开源素材，尽量清晰不要像素风格，与其他建筑风格保持一致）
  - 放置建筑，建筑的放置具有一定次序，从核心建筑（例如钟）到其他次要建筑
    - 放置村庄中心的钟
    - 从钟向各个方向延伸道路（可以考虑一些迷宫生成算法或者其他算法）
    - 在道路两旁放置房屋（各种类型房屋，不同种类房屋生成概率不同）
    - 另一种做法是先生成房屋，然后通过最小生成树算法（使用曼哈顿距离）将房屋联通起来
- 使用jmeter进行压力测试
