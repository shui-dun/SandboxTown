...
- 精灵刷新sprite_refresh表：
  - 精灵类型
  - 建筑类型（为null）表示该精灵可能在地图任意位置刷新
  - 最小刷新数目（例如对于稀有精灵这个数是一个负数）
  - 最大刷新数目
  - 刷新时间（白天、黄昏等）
...
- 移动端的问题（感觉难以解决，因此移动端可能只能聊天，不能进行游戏）：
  ...
  - 手机端主页背景（小球运动）卡顿
- 前端不支持级联主仆关系，即，狗b->狗a->我，此时狗a正常，但狗b的坐标没有正常同步到服务器（因为目前前端只上传自己、主人是自己、公共NPC的坐标）
- 一个精灵的owner会变化（例如主人死亡），但没有同步到前端
