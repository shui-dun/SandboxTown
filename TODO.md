...
- 已知的bug：当禁用根据方向键移动玩家的功能时，玩家与移动的野生狗（自己养的狗不会）发生碰撞之后，无法正常通过点击地图位置来移动（也就是说，后端能找到正确路径，但前端的补间动画无法工作）。这简直是莫名其妙，两个毫无关系的功能之间会互相影响，我怀疑是Phaser.js的上游问题。在找到原因之前，只好恢复根据方向键移动玩家的功能
