...
public interface ItemTypeEffectMapper {
    ...
    Set<ItemTypeEffect> selectByItemTypeAndOperation(ItemTypeEnum itemType, ItemOperationEnum operation);

    // 根据物品类型获得物品类型效果列表
    @Select("select * from item_type_effect where item_type=#{itemType}")
    Set<ItemTypeEffect> selectByItemType(ItemTypeEnum itemType);
}
