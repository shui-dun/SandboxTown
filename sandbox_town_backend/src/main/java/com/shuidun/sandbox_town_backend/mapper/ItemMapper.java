...
public interface ItemMapper {
    ...
    @Select("SELECT player_item.owner, player_item.item_id, player_item.item_count, item.name, item.description, item.basicPrice, item.usable, item.moneyInc, item.expInc, item.levelInc, item.hungerInc, item.hpInc, item.attackInc, item.defenseInc, item.speedInc\n" +
            "FROM player_item\n" +
            "INNER JOIN item\n" +
