...
public interface ItemTypeAttributeMapper {
    ...
    ItemTypeAttribute selectByItemTypeAndOperation(ItemTypeEnum itemType, ItemOperationEnum operation);

    // 根据物品类型获得物品类型属性列表
    @Select("select * from item_type_attribute where item_type=#{itemType}")
    Set<ItemTypeAttribute> selectByItemType(ItemTypeEnum itemType);
}
