...
我发现对于多人游戏，很多东西最好要在服务端实现，比如NPC之间的碰撞检测，但由于我使用的是springboot这种web框架，实现服务器端的碰撞检测比较麻烦。我打算还是在客户端检测碰撞，将事件发向服务器，当服务器检测到该事件后，进行处理，将结果发往所有客户端。另外，我们需要过滤重复的事件。碰撞者1id_碰撞者2id 作为key，放到redis里可以设置过期时间

思路2，改变游戏需求，改成交互性不强的游戏。因为这个游戏的目的主要是学习后端，而不是学习如何做游戏。具体来说，我们可以让玩家检测自己和自己的宠物的行为，自己或自己的宠物被怪物攻击了，才会向服务器发送这些事件，这样服务器就不会收到重复的事件，服务器的设计就变得单纯

例如碰撞监测，可以在客户端做，服务器用更低精度的模型和更低的频率来进行验算



帧同步就是干这个的。关于他的理论和实现，网上有很多，这里就不一一细讲。提几个比较少提到的

1.撇开服务器各自计算，最终的胜负结果服务器怎么判断。用投票机制，最终结果由各个客户端来投票完成，服务器汇总。如果其中一个客户端改数据了，最终结果必然和其他客户端的不一样

2. 随机数怎么解决，AI，战斗公式肯定会有用到随机数的地方。用一样的随机种子。同一个计算顺序，得出的随机序列也必然一样



帧同步的话，寻路和碰撞检测都放在客户端（观察者模式客户端和服务器各弄一套），关键帧发向服务器，服务器转发并且进行进一步的处理
